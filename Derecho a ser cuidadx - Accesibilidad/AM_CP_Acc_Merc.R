...
# Propósitos de AM_CP_Acc_Merc: 
# Calcular el indicador de accesibilidad de cuidados personales para personas
# adultas mayores desde el mercado.
...
# 6. Filtrar bases solo para estado de interés
enoe <- enoe %>%
  filter(ent == estado)
sdem <- sdem %>%
  filter(ent == estado)

# 7. Filtrar base PEA ocupada
pea <- enoe %>%
  filter(clase1 == 1) %>%
  filter(clase2 == 1)
...
personas_adultas_mayores <- sdem[sdem$eda>=65,] #Personas de 65 años o más
denominador <- sum(personas_adultas_mayores$fac, na.rm = TRUE)
...
AM_CP_Acc_Merc <- (numerador/denominador)
AM_CP_Acc_Merc

# 4. Guardar el resultado del indicador en la carpeta de output (como archivo .xlsx de Excel)
AM_CP_Acc_Merc <- as.data.frame(AM_CP_Acc_Merc)
write_xlsx(AM_CP_Acc_Merc, "output/AM_CP_Acc_Merc.xlsx")

...
AM_CS_Acc_Merc <- (numerador.1/denominador.1)*100
AM_CS_Acc_Merc

####################### Calcular información complementaria ####################

#1. Calcular denominador
denominador.2 <- sum(personas_adultas_mayores$factor, na.rm = TRUE)

#2. Clacular numerador
usa_seg_priv.2 <- personas_adultas_mayores[personas_adultas_mayores$sersalud==6 | personas_adultas_mayores$sersalud==7,]
numerador.2 <- sum(usa_seg_priv.2$factor, naa.rm = TRUE)

#3. Resultado indicador
AM_CS_Acc_Merc_Comp <- (numerador.2/denominador.2)*100
AM_CS_Acc_Merc_Comp

#Guardar los resultados de ambos indicadores en la carpeta de output como archivo .xlsx de Excel
AM_CS_Acc_Merc <- as.data.frame(AM_CS_Acc_Merc)
AM_CS_Acc_Merc_Comp <- as.data.frame(AM_CS_Acc_Merc_Comp)
write_xlsx(list(AM_CS_Acc_Merc, AM_CS_Acc_Merc_Comp), "output/AM_CS_Acc_Merc.xlsx")

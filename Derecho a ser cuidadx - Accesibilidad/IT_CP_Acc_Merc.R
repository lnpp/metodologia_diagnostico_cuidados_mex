...
# Propósitos de IT_CP_Acc_Merc: 
# Calcular el indicador de accesibilidad de cuidados personales para infancia
...
infancia_temprana <- sdem[sdem$eda>=0 & sdem$eda<=5,] #Personas de 0 a 5 años
denominador <- sum(infancia_temprana$fac)
denominador

#2. Calcular nominador
...
IT_CP_Acc_Merc <- (numerador/denominador)
IT_CP_Acc_Merc

# 4. Guardar el resultado del indicador en la carpeta de output (como archivo .xlsx de Excel)
IT_CP_Acc_Merc <- as.data.frame(IT_CP_Acc_Merc)
write_xlsx(IT_CP_Acc_Merc, "output/IT_CP_Acc_Merc.xlsx")

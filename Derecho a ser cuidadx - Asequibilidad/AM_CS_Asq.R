...
# Propósitos de AM_CS_Asq: 
# Calcular el indicador de asequibilidad de cuidados de salud
# para las personas adultas mayores, desde todos los proveedores.
...
# 2. Calcular numerador
  ...
  personas_adultas_mayores$prop_gasto_salud <- (personas_adultas_mayores$salud/personas_adultas_mayores$ing_cor)
  #Gastan 10 por ciento o menos
  no_onerosos <- personas_adultas_mayores[personas_adultas_mayores$prop_gasto_salud<=0.10,]
  numerador <- sum(no_onerosos$factor, na.rm = TRUE)

# 3. Resultado del indicador 
AM_CS_Asq <- (numerador/denominador)*100
AM_CS_Asq

# 4. Guardar el resultado del indicador en la carpeta de output (como archivo .xlsx de Excel)
AM_CS_Asq <- as.data.frame(AM_CS_Asq)
write_xlsx(AM_CS_Asq, "output/AM_CS_Asq.xlsx")

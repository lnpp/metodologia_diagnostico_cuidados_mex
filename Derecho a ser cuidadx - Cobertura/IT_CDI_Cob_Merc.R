...
# 2. Porporción 2 - cuidados por personas remuneradas en el hogar
  ...
  numerador.3 <- sum(cuid_remun$factor)
  #Proporción 2
  prop2 <- (numerador.3/denominador.2)

# 3. Proporción 3 - Casa hogar privada
  #Denominador
  ...
  numerador.4 <- sum(CAAS$r_t_0_4)
  #Proporción 3
  prop3 <- (numerador.4/denominador.3)

# 4. Resultado del indicador
IT_CDI_Cob_Merc <- (prop1 + prop2 + prop3)*100
IT_CDI_Cob_Merc

# 6. Guardar el resultado del indicador en la carpeta de output (como archivo .xlsx de Excel)
IT_CDI_Cob_Merc <- as.data.frame(IT_CDI_Cob_Merc) 
write_xlsx(IT_CDI_Cob_Merc, "output/IT_CDI_Cob_Merc.xlsx")

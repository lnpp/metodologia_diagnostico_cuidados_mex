...
# 2. Sustituir código en el siguiente objeto (siguiendo el formato correspondiente)
estado <- 6
...
IT_CP_Cob_Hog <- sum(p23_hogar_adultos$FACTOR)/sum(niños$FACTOR)*100
IT_CP_Cob_Hog

# 3. Guardar el resultado del indicador en la carpeta de output (como archivo .xlsx de Excel)
IT_CP_Cob_Hog <- as.data.frame(IT_CP_Cob_Hog) 
write_xlsx(IT_CP_Cob_Hog, "output/IT_CP_Cob_Hog.xlsx")
